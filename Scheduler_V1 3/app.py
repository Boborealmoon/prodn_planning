...
from TRIAL.trial_app.blocks import recalculate_all as embedded_trial_recalculate_all
from TRIAL.trial_app.db import DB_PATH as EMBEDDED_TRIAL_DB_PATH
from TRIAL.trial_app.db import db as embedded_trial_db
from TRIAL.trial_app.db import ensure_db as ensure_embedded_trial_db
from TRIAL.trial_app.imports import trial_data_management_stats as embedded_trial_data_management_stats
from TRIAL.trial_app.routes_flows import trial_prefixed_flows_bp as embedded_trial_flows_bp
from TRIAL.trial_app.routes_data_mgmt import data_mgmt_bp as embedded_trial_data_mgmt_bp
from TRIAL.trial_app.routes_gantt import trial_gantt_bp as embedded_trial_gantt_bp
from TRIAL.trial_app.routes_materials import materials_bp as embedded_trial_materials_bp
from TRIAL.trial_app.routes_process_sheets import process_sheets_bp as embedded_trial_process_sheets_bp
from TRIAL.trial_app.routes_summary import trial_summary_bp as embedded_trial_summary_bp
from TRIAL.trial_app.routes_trial import trial_bp as embedded_trial_bp
...
app.jinja_loader = ChoiceLoader(
    [
        PrefixLoader({"embedded_trial": FileSystemLoader(str(ROOT / "TRIAL" / "templates"))}),
        app.jinja_loader,
    ]
)


def ensure_embedded_trial_ready():
    ensure_embedded_trial_db()
    with embedded_trial_db() as con:
        embedded_trial_recalculate_all(con)


ensure_embedded_trial_ready()
app.register_blueprint(embedded_trial_bp)
app.register_blueprint(embedded_trial_data_mgmt_bp)
app.register_blueprint(embedded_trial_materials_bp)
app.register_blueprint(embedded_trial_gantt_bp)
app.register_blueprint(embedded_trial_summary_bp)
app.register_blueprint(embedded_trial_flows_bp)
app.register_blueprint(embedded_trial_process_sheets_bp)
...
@contextmanager
def trial_db():
    con = sqlite3.connect(TRIAL_DB_PATH)
    con.row_factory = sqlite3.Row
    con.execute("PRAGMA foreign_keys = ON")
    try:
        yield con
...
def apply_active_orders_statuses(con, sync_batch_id):
    # Active Orders is treated as an input signal, not a source that rewrites
    # the stored process_sheet status. We keep the current status on existing
    # rows and only use the uploaded list for import/debug plumbing.
    rows(con.execute("SELECT ps_id, part_no, bom_code FROM erp_active_orders_staging WHERE sync_batch_id = ?", (sync_batch_id,)))
    return 0, 0
...
def ensure_part_flow_step_columns(con):
    cols = [row[1] for row in con.execute("PRAGMA table_info(operation_seq)").fetchall()]
    if "share_setup_with_prev" in cols:
        try:
            con.execute("ALTER TABLE operation_seq DROP COLUMN share_setup_with_prev")
    ...
    for category in infer_machine_categories_from_step(con, step):
        ...
        if machine:
            if compact_text(step.get("machine_category")).upper() != category and category != "UNKNOWN":
                con.execute(
                    "UPDATE operation_seq SET machine_category = ? WHERE op_seq_id = ?",
                    (category, step["op_seq_id"]),
                )
...
def refresh_flow_step_machine_categories(con, machine_code=None):
    ...
    steps = rows(con.execute("SELECT op_seq_id, preferred_machine FROM operation_seq WHERE COALESCE(preferred_machine, '') <> ''"))
    for step in steps:
        con.execute(
            "UPDATE operation_seq SET machine_category = ? WHERE op_seq_id = ?",
            (machine_category_for(con, step["preferred_machine"]), step["op_seq_id"]),
        )
...
def upsert_bom_flow(con, part_id, part_no, bom_code):
    flow_code = bom_code
    flow = one(con.execute("SELECT * FROM bom_variation WHERE part_id = ? AND flow_code = ?", (part_id, flow_code)))
    if flow:
        bom_id = flow["bom_id"]
        if flow_erp_sync_locked(con, bom_id):
            return bom_id, 0
    else:
        cur = con.execute(
            "INSERT INTO bom_variation (part_id, flow_code, flow_name, is_default) VALUES (?, ?, ?, 1)",
            (part_id, flow_code, ""),
        )
        bom_id = cur.lastrowid
    con.execute("UPDATE bom_variation SET flow_name = '' WHERE bom_id = ?", (bom_id,))
    con.execute("UPDATE bom_variation SET is_default = CASE WHEN bom_id = ? THEN 1 ELSE 0 END WHERE part_id = ?", (bom_id, part_id))
    ...
    existing_by_op_seq_id = {step["op_seq_id"]: step for step in existing_steps}
    for idx, stage in enumerate(stage_rows, 1):
        ...
        if idx <= len(existing_steps):
            op_seq_id = existing_steps[idx - 1]["op_seq_id"]
            con.execute(
                """
                UPDATE operation_seq
    ...
    if len(existing_steps) > len(stage_rows):
        ...
        surplus_ids = [step["op_seq_id"] for step in surplus_steps]
        referenced_ids = {
            row["flow_op_seq_id"]
            for row in con.execute(
                f"SELECT DISTINCT flow_op_seq_id FROM planning_block WHERE flow_op_seq_id IN ({','.join('?' for _ in surplus_ids)})",
                surplus_ids,
            ).fetchall()
        } if surplus_ids else set()
        removable_ids = [op_seq_id for op_seq_id in surplus_ids if op_seq_id not in referenced_ids]
        if removable_ids:
            placeholders = ",".join("?" for _ in removable_ids)
            con.execute(
                f"DELETE FROM operation_seq WHERE op_seq_id IN ({placeholders})",
                removable_ids,
            )
        if referenced_ids:
            # Preserve any still-referenced legacy steps so existing planning rows keep valid foreign keys.
            con.execute(
                "UPDATE operation_seq SET is_last_op = 0 WHERE op_seq_id IN ({})".format(",".join("?" for _ in referenced_ids)),
                list(referenced_ids),
            )
    return bom_id, len(stage_rows)
...
def merge_erp_process_sheets(con, sync_batch_id):
    ...
    for row in staged_rows:
        ...
        con.execute(
            """
            INSERT INTO process_sheet (ps_id, source_ps_id, pp_partial_no, part_id, part_no, part_desc, order_date, due_date, total_qty, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                ps_id,
...
def merge_erp_bom_flows(con):
    ...
    for route in route_rows:
        ...
        existing_flow = one(
            con.execute(
                "SELECT bom_id FROM bom_variation WHERE part_id = ? AND flow_code = ?",
                (part_id, bom_code),
            )
        )
        if existing_flow and flow_erp_sync_locked(con, int(existing_flow["bom_id"])):
            skipped += 1
            flow_by_key[key] = int(existing_flow["bom_id"])
            continue
        stage_count = one(
            con.execute(
        ...
        if stage_count <= 0:
            ...
            skipped += 1
            continue
        bom_id, _ = upsert_bom_flow(con, part_id, part_no, bom_code)
        flow_by_key[key] = bom_id
        if existing_flow:
            updated += 1
        else:
            inserted += 1
    ...
    ps_rows = rows(
        con.execute(
            """
            SELECT ps.ps_id, ps.part_id, ps.part_no AS part_no, ps.selected_bom_id, l.bom_code
            FROM process_sheet ps
            JOIN process_sheet_erp_link l ON l.ps_id = ps.ps_id
            WHERE COALESCE(l.bom_code, '') <> '' AND COALESCE(ps.part_no, '') <> ''
            """
        )
    )
    for ps in ps_rows:
        key = (compact_text(ps["part_no"]), compact_text(ps["bom_code"]))
        bom_id = flow_by_key.get(key)
        if bom_id and ps_erp_sync_locked(con, ps["ps_id"]):
            skipped += 1
            continue
        if bom_id and ps["selected_bom_id"] != bom_id:
            con.execute(
                "UPDATE process_sheet SET selected_bom_id = ?, updated_at = CURRENT_TIMESTAMP WHERE ps_id = ?",
                (bom_id, ps["ps_id"]),
            )
    return inserted, updated, skipped
...
TRIAL_PROFILE_BY_SHIFT = {
    "NORMAL": "NORMAL_DAY",
    "STANDARD": "NORMAL_DAY",
    "DAY": "NORMAL_DAY",
    "DAY_SHIFT": "NORMAL_DAY",
    "NORMAL_DAY": "NORMAL_DAY",
    "NORMAL_DAY_NIGHT": "NORMAL_DAY_NIGHT",
    "DAY_NIGHT": "NORMAL_DAY_NIGHT",
    "NIGHT": "NORMAL_DAY_NIGHT",
    "SAT": "SATURDAY",
    "SATURDAY": "SATURDAY",
    "24H": "FULL_24H",
    "24HR": "FULL_24H",
...
def trial_default_profile_name_for_date(work_date):
...
def trial_profile_name_for_machine(machine, work_date=None):
    text = compact_text((machine or {}).get("shift_profile") or "").upper()
    if not text:
        return trial_default_profile_name_for_date(work_date or date.today())
    if text in {"STANDARD", "NORMAL", "DAY", "DAY_SHIFT"}:
        return trial_default_profile_name_for_date(work_date or date.today())
    if text in {"NORMAL_DAY", "NORMAL_DAY_NIGHT", "SATURDAY", "FULL_24H", "OFF"}:
        return text
    for key, profile_name in TRIAL_PROFILE_BY_SHIFT.items():
        if key in text:
            return profile_name
    if "24" in text:
        return "FULL_24H"
...
def trial_dt_text(value):
    if not value:
        return ""
    return value.isoformat(sep=" ", timespec="seconds")


def trial_day_start_dt(work_date, start_minute):
    work_date = work_date if isinstance(work_date, date) else date.fromisoformat(str(work_date))
    start_minute = int(start_minute or 0)
    hour = max(0, start_minute // 60)
    minute = max(0, start_minute % 60)
    return datetime.combine(work_date, dt_time(hour=hour, minute=minute))


def capacity_profile_row(con, profile_name):
    return one(
        con.execute(
            "SELECT profile_id, profile_name, capacity_minutes, start_minute, note FROM capacity_profile WHERE profile_name = ?",
            (profile_name,),
        )
    )


def trial_capacity_for_day(con, machine, work_date):
    machine_id = int((machine or {}).get("machine_id") or 0)
    work_date = work_date.isoformat() if isinstance(work_date, date) else compact_text(work_date)
    override = one(
        con.execute(
            """
            SELECT tmd.day_id, tmd.machine_id, tmd.work_date, tmd.profile_id, tmd.capacity_minutes, tmd.start_minute,
                   tcp.profile_name, COALESCE(tmd.note, tcp.note, '') AS note
            FROM machine_capacity_day tmd
            JOIN capacity_profile tcp ON tcp.profile_id = tmd.profile_id
            WHERE tmd.work_date = ?
            ORDER BY tmd.machine_id
            LIMIT 1
            """,
            (work_date,),
        )
    ...
    if override:
        return override
    profile_name = trial_default_profile_name_for_date(work_date)
    profile = capacity_profile_row(con, profile_name) or capacity_profile_row(con, "NORMAL_DAY") or {}
    return {
        "day_id": None,
        "machine_id": machine_id,
        "work_date": work_date,
        "profile_id": profile.get("profile_id"),
        "capacity_minutes": int(profile.get("capacity_minutes") or 0),
        "start_minute": int(profile.get("start_minute") or 510),
        "profile_name": profile.get("profile_name") or profile_name,
        "note": profile.get("note") or "",
    }


def trial_next_day_start_dt(con, machine, work_date):
    info = trial_capacity_for_day(con, machine, work_date)
    return trial_day_start_dt(work_date, info.get("start_minute") or 0)


def trial_block_duration_minutes(operation, block):
    scheduled_qty = float((block or {}).get("scheduled_qty") or 0)
    actual_qty = float((block or {}).get("actual_good_qty") or 0)
    status = str((block or {}).get("status") or "").upper()
    if status == "IN_PROGRESS" and 0 < actual_qty < scheduled_qty:
        scheduled_qty = max(0.0, scheduled_qty - actual_qty)
    setup_minutes = float((operation or {}).get("setup_minutes") or 0) if int((block or {}).get("include_setup") or 0) == 1 else 0.0
    cycle_time = float((operation or {}).get("cycle_minutes_per_qty") or 0)
    return max(0.0, setup_minutes + (scheduled_qty * cycle_time))


def trial_block_status(block):
    scheduled = float((block or {}).get("scheduled_qty") or 0)
    actual = float((block or {}).get("actual_good_qty") or 0)
    if scheduled and actual >= scheduled:
        return "DONE"
    if actual > 0:
        return "IN_PROGRESS"
    return "PLANNED"
...
def trial_last_actual_report_date(con, block_id):
    row = one(
        con.execute(
            "SELECT MAX(report_date) AS report_date FROM production_actual WHERE block_id = ?",
            (int(block_id),),
        )
    )
    report_date = compact_text((row or {}).get("report_date")) if row else ""
    if not report_date:
        return None
    try:
...
def trial_allocate_block_segments(con, machine, block, start_dt):
    cycle_time = float((block or {}).get("cycle_minutes_per_qty") or 0)
    scheduled_qty = int(max(0, round(float((block or {}).get("scheduled_qty") or 0))))
    actual_qty = int(max(0, round(float((block or {}).get("actual_good_qty") or 0))))
    status = str((block or {}).get("status") or "").upper()
    is_tail = status == "IN_PROGRESS" and 0 < actual_qty < scheduled_qty
    setup_minutes = float((block or {}).get("setup_minutes") or 0) if int((block or {}).get("include_setup") or 0) == 1 and not is_tail else 0.0
    qty_remaining = max(0, scheduled_qty - actual_qty) if is_tail else scheduled_qty
    cursor = start_dt
    segments = []
    safety = 0
    setup_remaining = float(setup_minutes or 0)
    if is_tail:
        last_actual_date = trial_last_actual_report_date(con, block.get("block_id"))
        if last_actual_date:
            tail_start = trial_next_day_start_dt(con, machine, last_actual_date + timedelta(days=1))
            if tail_start > cursor:
                cursor = tail_start
    while setup_remaining > 0:
        safety += 1
        if safety > 5000:
            raise RuntimeError("Trial scheduler timed out while allocating capacity.")
        work_date = cursor.date()
        info = trial_capacity_for_day(con, machine, work_date)
        cap_minutes = int(info.get("capacity_minutes") or 0)
        day_start = trial_day_start_dt(work_date, info.get("start_minute") or 0)
        if cap_minutes <= 0:
            cursor = trial_next_day_start_dt(con, machine, work_date + timedelta(days=1))
            continue
        day_end = day_start + timedelta(minutes=cap_minutes)
        if cursor < day_start:
            cursor = day_start
        if cursor >= day_end:
            cursor = trial_next_day_start_dt(con, machine, work_date + timedelta(days=1))
        ...
        used = min(available, int(round(setup_remaining)))
        if used <= 0:
            break
        seg_end = cursor + timedelta(minutes=used)
        segments.append(
            {
                "segment_date": work_date.isoformat(),
                "segment_type": "setup",
                "qty_done": 0.0,
                "minutes_used": used,
                "start_datetime": trial_dt_text(cursor),
                "end_datetime": trial_dt_text(seg_end),
                "is_actual": 0,
            }
        )
        setup_remaining -= used
        cursor = seg_end
    ...
    while qty_remaining > 0:
        ...
        if cycle_time <= 0:
            break
        pieces = min(qty_remaining, int(available // cycle_time))
        if pieces <= 0:
            cursor = trial_next_day_start_dt(con, machine, work_date + timedelta(days=1))
            continue
        used = pieces * cycle_time
        seg_end = cursor + timedelta(minutes=used)
        segments.append(
            {
                "segment_date": work_date.isoformat(),
                "segment_type": "production",
                "qty_done": float(pieces),
                "minutes_used": used,
                "start_datetime": trial_dt_text(cursor),
                "end_datetime": trial_dt_text(seg_end),
                "is_actual": 0,
...
def trial_clear_block_segments(con, block_id):
    con.execute("DELETE FROM run_block_segment WHERE block_id = ?", (int(block_id),))


def trial_sync_block_segments(con, machine, operation, block, segments):
    block_id = int(block["block_id"])
    trial_clear_block_segments(con, block_id)
    for seg in segments:
        con.execute(
            """
            INSERT INTO run_block_segment (
              block_id, machine_id, segment_date, segment_type, qty_done, minutes_used, start_datetime, end_datetime, is_actual
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                ...
                seg["segment_date"],
                seg["segment_type"],
                float(seg["qty_done"] or 0),
                float(seg["minutes_used"] or 0),
                seg["start_datetime"],
                seg["end_datetime"],
                int(seg.get("is_actual") or 0),
            ),
        )


def trial_recalculate_machine(con, machine_id):
    machine = one(con.execute("SELECT * FROM machines WHERE machine_id = ?", (int(machine_id),)))
    if not machine:
        return []
    blocks = rows(
        con.execute(
            """
            SELECT b.*, o.job_no, o.operation_name, o.total_qty, o.setup_minutes, o.cycle_minutes_per_qty, o.compatible_machine_group
            FROM run_block b
    ...
    if not blocks:
        return []
    current_dt = trial_next_day_start_dt(con, machine, date.today())
    updated = []
    for idx, block in enumerate(blocks, 1):
        if int(block.get("queue_position") or 0) != idx:
            con.execute("UPDATE run_block SET queue_position = ? WHERE block_id = ?", (idx, int(block["block_id"])))
            block["queue_position"] = idx
        anchor_dt = trial_parse_dt(block.get("anchor_datetime"))
        if anchor_dt:
            if current_dt <= anchor_dt or idx == 1:
                current_dt = anchor_dt
            else:
                con.execute("UPDATE run_block SET anchor_datetime = '' WHERE block_id = ?", (int(block["block_id"]),))
        start_dt = current_dt
        segments, end_dt = trial_allocate_block_segments(con, machine, block, start_dt)
        trial_sync_block_segments(con, machine, block, block, segments)
        status = trial_block_status(block)
        con.execute(
            """
            UPDATE run_block
            SET calculated_start_datetime = ?, calculated_end_datetime = ?, status = ?, updated_at = CURRENT_TIMESTAMP
            WHERE block_id = ?
            """,
            (trial_dt_text(start_dt), trial_dt_text(end_dt), status, int(block["block_id"])),
        )
        block["calculated_start_datetime"] = trial_dt_text(start_dt)
        block["calculated_end_datetime"] = trial_dt_text(end_dt)
        block["status"] = status
        current_dt = end_dt
...
def trial_recalculate_all(con):
    machine_ids = [row["machine_id"] for row in rows(con.execute("SELECT DISTINCT machine_id FROM run_block ORDER BY machine_id"))]
    for machine_id in machine_ids:
        trial_recalculate_machine(con, machine_id)


def trial_machine_payload(con, machine, block_map=None, capacity_map=None):
    machine_id = int(machine["machine_id"])
    blocks = (block_map or {}).get(machine_id, [])
    capacity_days = (capacity_map or {}).get(machine_id, [])
    return {
...
def trial_load_capacity_window(con, machine_ids, start_date=None, days=7):
    start = date.fromisoformat(start_date) if start_date else date.today()
    end = start + timedelta(days=max(1, int(days)) - 1)
    capacity_rows = rows(
        con.execute(
            """
            SELECT tmd.*, tcp.profile_name
            FROM machine_capacity_day tmd
            JOIN capacity_profile tcp ON tcp.profile_id = tmd.profile_id
            WHERE tmd.machine_id IN ({})
              AND tmd.work_date BETWEEN ? AND ?
            ORDER BY tmd.machine_id, tmd.work_date
            """.format(",".join("?" for _ in machine_ids) if machine_ids else "0"),
            ([*machine_ids, start.isoformat(), end.isoformat()] if machine_ids else [start.isoformat(), end.isoformat()]),
        )
    ) if machine_ids else []
    grouped = {}
    for row in capacity_rows:
        grouped.setdefault(int(row["machine_id"]), []).append(row)
    return grouped


def operation_row(con, operation_id):
    return one(con.execute("SELECT * FROM operation WHERE operation_id = ?", (int(operation_id),)))
    ...
    return one(
        con.execute(
            ...
            SELECT b.*, o.job_no, o.operation_name, o.total_qty, o.setup_minutes, o.cycle_minutes_per_qty,
                   ...
                   m.machine_code, m.shift_profile
            FROM run_block b
            JOIN operation o ON o.operation_id = b.operation_id
...
def trial_actual_totals_by_block(con, block_id=None):
    if block_id is None:
        return {
            int(row["block_id"]): {
                "actual_good_qty": float(row["actual_good_qty"] or 0),
                "actual_reject_qty": float(row["actual_reject_qty"] or 0),
            }
            for row in rows(
                con.execute(
                    """
                    SELECT block_id, SUM(actual_good_qty) AS actual_good_qty, SUM(actual_reject_qty) AS actual_reject_qty
    ...
    row = one(
        con.execute(
            """
            SELECT COALESCE(SUM(actual_good_qty), 0) AS actual_good_qty,
                   COALESCE(SUM(actual_reject_qty), 0) AS actual_reject_qty
            FROM production_actual
            WHERE block_id = ?
            """,
            (int(block_id),),
    ...
    return {
        "actual_good_qty": float(row["actual_good_qty"] or 0) if row else 0.0,
        "actual_reject_qty": float(row["actual_reject_qty"] or 0) if row else 0.0,
    }
...
def trial_block_payload(block):
    ...
    return {
        ...
        "queue_position": block["queue_position"],
        ...
        "job_no": block["job_no"],
        "operation_name": block["operation_name"],
        ...
        "source_ps_id": block.get("source_ps_id") or "",
        "source_op_seq_id": int(block.get("source_op_seq_id") or 0),
        "source_op_no": block.get("source_op_no") or "",
        "machine_code": block["machine_code"],
        "shift_profile": block["shift_profile"],
    }


def trial_catalog_items(con, include_completed=False):
    used_sources = {
        (compact_text(row.get("source_ps_id")), int(row.get("source_op_seq_id") or 0))
        for row in rows(
            con.execute(
                """
                SELECT DISTINCT source_ps_id, source_op_seq_id
                FROM operation
                WHERE COALESCE(source_ps_id, '') <> '' AND COALESCE(source_op_seq_id, 0) > 0
                """
            )
        )
    }
    records = rows(
        con.execute(
            """
            SELECT ps.ps_id, ps.part_no, ps.part_desc, ps.due_date, ps.total_qty, ps.status, ps.planner_status, ps.selected_bom_id,
                   p.part_name, pfs.op_seq_id, pfs.seq_no, pfs.op_no, pfs.op_type, pfs.machine_category, pfs.preferred_machine,
    ...
    for row in records:
        ...
        item = grouped.setdefault(
            ps_id,
            {
                ...
                "planner_status": row.get("planner_status") or "",
        ...
        op_item = {
            ...
            "source_op_no": row.get("op_no") or "",
            "op_no": row.get("op_no") or "",
            "op_type": row.get("op_type") or "",
            "machine_category": row.get("machine_category") or "",
            "preferred_machine": row.get("preferred_machine") or "",
            "cycle_time": float(row.get("cycle_time") or 0),
            "setup_time": float(row.get("setup_time") or 0),
            "is_last_op": int(row.get("is_last_op") or 0),
            "job_no": ps_id,
            "operation_name": f"{row.get('op_no') or ''} {row.get('op_type') or ''}".strip(),
            "total_qty": float(row.get("total_qty") or 0),
            "compatible_machine_group": row.get("machine_category") or "",
        }
        item["all_ops"].append(op_item)
        if (ps_id, op_seq_id) not in used_sources:
...
def ensure_db():
    ...
    with db() as con:
        ...
        if planning_row_cols and "flow_op_seq_id" not in planning_row_cols:
            con.execute("ALTER TABLE planning_row ADD COLUMN flow_op_seq_id INTEGER NOT NULL DEFAULT 0")
        ...
        if any(row[1] == legacy_block_col and int(row[3] or 0) == 1 for row in planning_row_info):
            ...
            con.execute(
                ...
                INSERT INTO planning_row (
                    ...
                    setup_mins, locked, seq_no, op_no, machine_id, machine_code, flow_op_seq_id, split_group_id, split_piece_id
                )
                SELECT
                    ...
                    COALESCE(machine_id, 0), COALESCE(machine_code, ''), COALESCE(flow_op_seq_id, 0), '', ''
        ...
        if planning_envelope_cols:
            con.execute(
                ...
                INSERT INTO planning_envelope (
                    ps_id, seq_no, op_no, machine_id, machine_code, flow_op_seq_id, split_group_id, split_piece_id,
                    total_qty, locked, archived, envelope_start, envelope_end
                )
                SELECT pb.ps_id, pb.seq_no, pb.op_no, pb.machine_id, pb.machine_code, pb.flow_op_seq_id,
            ...
            con.execute(
                ...
                SET envelope_id = COALESCE((
                    ...
                    WHERE pe.ps_id = planning_row.ps_id
                      ...
                      AND pe.flow_op_seq_id = planning_row.flow_op_seq_id
        ...
        if history_row_cols and "flow_op_seq_id" not in history_row_cols:
            con.execute("ALTER TABLE history_row ADD COLUMN flow_op_seq_id INTEGER NOT NULL DEFAULT 0")
        ...
        if history_envelope_cols:
            con.execute(
                ...
                INSERT OR IGNORE INTO history_envelope (
                    ps_id, seq_no, op_no, machine_id, machine_code, flow_op_seq_id,
                    total_qty, actual_out, locked, archived_at
                )
                SELECT hr.ps_id, hr.seq_no, hr.op_no, COALESCE(hr.machine_id, 0), hr.machine_code, COALESCE(hr.flow_op_seq_id, 0),
                       COALESCE(SUM(hr.qty), 0), COALESCE(SUM(hr.actual_out), 0), 0, CURRENT_TIMESTAMP
                FROM history_row hr
                GROUP BY hr.ps_id, hr.seq_no, hr.op_no, hr.machine_id, hr.machine_code, hr.flow_op_seq_id
                """
            )
            con.execute(
                """
                ...
                SET hist_envelope_id = COALESCE((
                    ...
                    WHERE he.ps_id = history_row.ps_id
                      ...
                      AND he.flow_op_seq_id = history_row.flow_op_seq_id
                    LIMIT 1
                ), hist_envelope_id)
        ...
        operation_cols = [row[1] for row in con.execute("PRAGMA table_info(operation)").fetchall()]
        if operation_cols and "source_ps_id" not in operation_cols:
            con.execute("ALTER TABLE operation ADD COLUMN source_ps_id TEXT DEFAULT ''")
        if operation_cols and "source_op_seq_id" not in operation_cols:
            con.execute("ALTER TABLE operation ADD COLUMN source_op_seq_id INTEGER NOT NULL DEFAULT 0")
        if operation_cols and "source_op_no" not in operation_cols:
            con.execute("ALTER TABLE operation ADD COLUMN source_op_no TEXT DEFAULT ''")
        existing_profiles = {row["profile_name"] for row in rows(con.execute("SELECT profile_name FROM capacity_profile"))}
        trial_profiles = [
            ("NORMAL_DAY", 510, 510, "Default day shift"),
            ("NORMAL_DAY_NIGHT", 630, 510, "Day + night coverage"),
            ("SATURDAY", 420, 510, "Short Saturday shift"),
            ("FULL_24H", 1440, 0, "24 hour coverage"),
            ("OFF", 0, 0, "No planned capacity"),
        ]
        for profile_name, capacity_minutes, start_minute, note in trial_profiles:
            if profile_name not in existing_profiles:
                con.execute(
                    "INSERT INTO capacity_profile (profile_name, capacity_minutes, start_minute, note) VALUES (?, ?, ?, ?)",
...
def flow_erp_sync_locked(con, bom_id):
    row = one(con.execute("SELECT erp_sync_locked FROM part_flow_local_override WHERE bom_id = ?", (bom_id,)))
...
def mark_flow_erp_sync_locked(con, bom_id, locked=1):
    con.execute(
        """
        INSERT INTO part_flow_local_override (bom_id, erp_sync_locked, updated_at)
        VALUES (?, ?, CURRENT_TIMESTAMP)
        ON CONFLICT(bom_id) DO UPDATE SET
          erp_sync_locked = excluded.erp_sync_locked,
          updated_at = CURRENT_TIMESTAMP
        """,
        (bom_id, int(bool(locked))),
    )
...
def backfill_erp_sync_locks(con):
    ...
    ps_rows = rows(con.execute("SELECT ps_id, selected_bom_id, total_qty, status FROM process_sheet ORDER BY ps_id"))
    for ps in ps_rows:
        ps_id = ps["ps_id"]
        if not ps_id:
            continue
    ...
    bom_ids = [
        ...
        for row in rows(
            con.execute(
                """
                SELECT DISTINCT ps.selected_bom_id
                FROM process_sheet ps
    ...
    for bom_id in sorted(set(bom_ids)):
        before = flow_erp_sync_locked(con, bom_id)
        mark_flow_erp_sync_locked(con, bom_id, 1)
        if not before:
            locked_flows += 1
...
def step_completion_override(con, ps_id, op_seq_id):
    row = one(
        con.execute(
            "SELECT force_completed FROM process_sheet_step_local_override WHERE ps_id = ? AND op_seq_id = ?",
            (ps_id, op_seq_id),
        )
    )
    if not row:
...
def step_force_completed(con, ps_id, op_seq_id):
    return step_completion_override(con, ps_id, op_seq_id) == "COMPLETED"
...
def normalize_active_sheet_status(ps, status, status_reason, status_flags):
    stored_status = compact_text(ps.get("status")).upper()
    if stored_status != "ACTIVE" or status != "COMPLETED":
        return status, status_reason, status_flags
    status = "NEEDS_REVIEW"
    status_reason = "Active sheet reached completed state and needs review"
    flags = [flag for flag in status_flags if flag != "ERP_COMPLETE"]
    if "COMPLETION_REVIEW" not in flags:
        flags.append("COMPLETION_REVIEW")
    return status, status_reason, flags
...
def process_sheet_step_metrics(con, ps_id, steps):
    ...
    for row in timeline_rows:
        op_seq_id = int(row.get("flow_op_seq_id") or 0)
        active_by_step.setdefault(op_seq_id, []).append(row)
    hist_rows = rows(
        con.execute(
            """
            SELECT
    ...
    for step in steps:
        active_rows = active_by_step.get(int(step["op_seq_id"]), [])
        active = {
    ...
    return {
        ...
        "last_op_op_seq_id": last.get("op_seq_id"),
...
def compute_process_sheet_status(con, ps, erp_index=None):
    ...
    if manual_override == "COMPLETED":
    ...
    elif not erp_index["available"]:
        stored_status = compact_text(ps.get("status")).upper()
        status = stored_status or "UNPLANNED"
        status_reason = "ERP active export unavailable; preserving stored process sheet status"
        status_flags.append("LOCAL_PRESERVED_STATUS")
        ...
        if not erp_active_match:
            ...
            elif overall_planned_qty <= 0:
                status = "UNPLANNED"
                status_reason = "ERP active export no longer lists this sheet"
                status_flags.append("ERP_NOT_PLANNED")
            else:
                status = "UNPLANNED"
                status_reason = "ERP active export no longer lists this sheet"
                status_flags.append("ERP_NOT_PLANNED")
    ...
    status, status_reason, status_flags = normalize_active_sheet_status(ps, status, status_reason, status_flags)
    ...
    debug = {
        ...
        "last_op_op_seq_id": step_metrics["last_op_op_seq_id"],
...
def snapshot_process_sheet_progress(con, ps_id, reason="MANUAL_RESET"):
    ...
    step_overrides = rows(
        con.execute(
            """
            SELECT op_seq_id, force_completed
    ...
    payload = {
        ...
        "step_overrides": [
            {"op_seq_id": int(row["op_seq_id"]), "force_completed": int(row.get("force_completed") or 0)}
...
def reset_process_sheet_progress(con, ps_id):
    ...
    for op_seq_id in op_seq_ids:
        con.execute(
            """
            INSERT INTO process_sheet_step_local_override (ps_id, op_seq_id, force_completed, updated_at)
            VALUES (?, ?, 0, CURRENT_TIMESTAMP)
            """,
            (ps_id, op_seq_id),
        )
...
def restore_process_sheet_progress(con, ps_id, snapshot_id=None):
    ...
    for row in payload.get("step_overrides", []):
        con.execute(
            ...
            VALUES (?, ?, ?, CURRENT_TIMESTAMP)
            """,
            (ps_id, int(row["op_seq_id"]), int(row.get("force_completed") or 0)),
        )
...
def flow_steps(con, bom_id):
    ensure_part_flow_step_columns(con)
    return rows(
        con.execute(
            """
            SELECT op_seq_id, seq_no AS seq, op_no, op_type, machine_category, preferred_machine,
                   cycle_time, setup_time, is_last_op
            FROM operation_seq WHERE bom_id = ? ORDER BY seq_no, op_seq_id
            """,
            (bom_id,),
        )
    )
...
def route_label(con, bom_id):
    if not bom_id:
        return ""
    return ", ".join(step["op_no"] for step in flow_steps(con, bom_id))
...
def compute_process_sheet_status_light(con, ps, erp_index=None, cache=None):
    ...
    if steps:
        for step in steps:
            ...
            completed = step_completion_override(con, ps_id, step["op_seq_id"]) == "COMPLETED" or actual_qty >= total_qty
        ...
        last_op_seq_id = last_step.get("op_seq_id")
    ...
    debug = {
        ...
        "last_op_op_seq_id": last_op_seq_id,
...
def step_progress(con, ps_id, step, total_qty):
    ...
    seq_no = step["seq"]
    flow_op_seq_id = step["op_seq_id"]
    active_rows = [r for r in ps_timeline_rows(con, ps_id) if int(r.get("flow_op_seq_id") or 0) == int(flow_op_seq_id)]
    active = {
        "planned_qty": sum(float(r.get("qty") or 0) for r in active_rows),
...
def planner_status(con, ps, totals):
    return compact_text(ps.get("status")).upper() or compute_process_sheet_status(con, ps)["planner_status"]


def warnings_for(con, ps, totals, material_cache=None, support_sync=True, schedule=None, status_value=None):
    status_info = totals if isinstance(totals, dict) and totals.get("planner_status") else compute_process_sheet_status(con, ps)
    current_status = compact_text(status_value or ps.get("status")).upper()
    if current_status == "COMPLETED":
        return []
    if float(ps["total_qty"] or 0) <= 0:
        return []
    warnings = []
    if ps["due_date"] and ps["due_date"] < date.today().isoformat() and current_status != "COMPLETED":
...
def step_statuses(con, ps):
    steps = flow_steps(con, ps["selected_bom_id"])
    planned = {r["flow_op_seq_id"] for r in con.execute("SELECT DISTINCT flow_op_seq_id FROM planning_block WHERE ps_id = ? AND archived = 0", (ps["ps_id"],))}
    ...
    for step in steps:
        ...
        step["completion_override"] = step_completion_override(con, ps["ps_id"], step["op_seq_id"])
        ...
        if step["force_completed"]:
            ...
            step["op_status"] = "COMPLETED"
        elif step["op_seq_id"] in planned:
...
def contiguous_envelope_for_row(con, timeline_rows, anchor_row_id):
    ...
    anchor_key = (
        ...
        int(anchor.get("flow_op_seq_id") or 0),
    )
    ...
    lane_rows = [
        ...
        if (
            ...
            and int(row.get("flow_op_seq_id") or 0) == anchor_key[4]
        )
    ]
    lane_index = next(
    ...
    def same_envelope(prev_row, next_row):
        return (
            ...
            and int(prev_row.get("flow_op_seq_id") or 0) == anchor_key[4]
            and int(next_row.get("flow_op_seq_id") or 0) == anchor_key[4]
...
def normalize_standard_rows(con, row_ids):
    ...
    if machine and machine.get("shift_profile") == "24HR":
        return 0
    step = one(con.execute("SELECT cycle_time FROM operation_seq WHERE op_seq_id = ?", (int(rows_for_group[0]["flow_op_seq_id"]),)))
...
def preceding_envelope_allows_setup_carryover(con, ps_id, machine_id, start_date, start_min, step=None, ignore_envelope_id=None):
    op_seq_id = int(step["op_seq_id"]) if step and step.get("op_seq_id") is not None else None
    if not ps_id or not machine_id or op_seq_id is None:
        return False
    ...
    timeline_rows = rows(
        con.execute(
            ...
            WHERE ps.part_id = ?
              AND pr.machine_id = ?
              AND pr.flow_op_seq_id = ?
            ORDER BY pr.plan_date, pr.start_min, pr.end_min, pr.row_id
            """,
            (int(ps["part_id"]), int(machine_id), op_seq_id),
        )
    )
    earlier_rows = [
...
def create_block_for_rows(con, ps_id, step, machine, rows_for_block, setup_charged=None):
    ...
    envelope_row = one(
        con.execute(
            ...
            WHERE ps_id = ? AND seq_no = ? AND op_no = ? AND machine_id = ? AND machine_code = ? AND flow_op_seq_id = ?
    ...
    if envelope_row:
        ...
        con.execute(
            """
            INSERT INTO planning_envelope (
                ps_id, seq_no, op_no, machine_id, machine_code, flow_op_seq_id, split_piece_id,
                total_qty, locked, archived, envelope_start, envelope_end, split_group_id
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, 0, ?, ?, ?)
            """,
            (
    ...
    cur = con.execute(
        """
        INSERT INTO planning_block (ps_id, flow_op_seq_id, op_no, seq_no, machine_id, machine_code,
    ...
    for row in rows_for_block:
        ...
        con.execute(
            """
            INSERT INTO planning_row (envelope_id, split_piece_id, ps_id, plan_date, start_min, end_min, qty, setup_mins, seq_no, op_no, machine_id, machine_code, flow_op_seq_id, split_group_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (envelope_id, split_piece_id, ps_id, row["plan_date"], row["start_min"], row["end_min"], row["qty"], setup_mins, seq_value, step["op_no"], machine["machine_id"], machine["machine_code"], step["op_seq_id"], split_group_id),
        )
...
def blocks_for_ps(con, ps_id):
    ...
    for row in timeline_rows:
        lane_key = (
            ...
            int(row.get("flow_op_seq_id") or 0),
        )
...
def serialize_ps(con, ps, detail=False, material_cache=None, erp_index=None, list_cache=None):
    ...
    item = dict(ps)
    item.update(totals)
    item["stored_status"] = compact_text(ps.get("status")).upper()
    item["computed_planner_status"] = totals["planner_status"]
    ...
    item["warnings"] = warnings_for(
        ...
        support_sync=detail,
        schedule=span,
        status_value=item["stored_status"] or item["planner_status"],
    )
    ...
    if detail:
        ...
        item["route_label"] = route_label(con, ps["selected_bom_id"])
...
def occupied_rows(con, machine_id, plan_date, ignore_envelope_id=None, ignore_row_ids=None):
    sql = """
        SELECT row_id, envelope_id, ps_id, plan_date, start_min, end_min, qty, actual_out, actual_out_set,
               seq_no, op_no, machine_id, machine_code, flow_op_seq_id, split_group_id
        FROM planning_row
...
def existing_step_span(con, ps_id, op_seq_id):
    return one(
        con.execute(
            """
            SELECT pr.flow_op_seq_id,
            ...
            FROM planning_row pr
            WHERE pr.ps_id = ? AND pr.flow_op_seq_id = ?
            GROUP BY pr.flow_op_seq_id
...
def replan_downstream(con, ps_id, from_seq, earliest_date, earliest_min, carry_mode="weekdays", split_piece_id=None):
    ...
    def plan_shared_chain(chain, chain_machine, start_date, start_min):
        chain_remaining = {
            chain_step["op_seq_id"]: remaining_qty_for_step(chain_step)
        ...
        while any(qty > 0 for qty in chain_remaining.values()):
            ...
            active_steps = [s for s in chain if chain_remaining.get(s["op_seq_id"], 0) > 0]
            if not active_steps:
                break
            ...
            if rounds <= 0:
                ...
                for chain_step in active_steps:
                    step_qty = chain_remaining[chain_step["op_seq_id"]]
                    ...
                    if available_minutes_in_day(chain_machine, chain_date, chain_min) >= setup_here + step_cycle:
                        ...
                        if qty_alloc > 0:
                            ...
                            chain_remaining[chain_step["op_seq_id"]] -= qty_alloc
            ...
            for chain_step in active_steps:
                qty_alloc = min(chain_remaining[chain_step["op_seq_id"]], chain_round_qty)
                if qty_alloc <= 0:
                    continue
                actual_start_date, actual_start_min = first_schedulable_start(
            ...
            while available_minutes_in_day(chain_machine, chain_date, chain_min) > 0 and any(qty > 0 for qty in chain_remaining.values()):
                progressed = False
                for chain_step in active_steps:
                    remaining = chain_remaining[chain_step["op_seq_id"]]
                    if remaining <= 0:
                        continue
                    step_cycle = float(chain_step["cycle_time"] or 1)
...
def apply_flow_timing_updates(con, bom_id, from_seq=None):
    affected = 0
    skipped = []
    ps_rows = rows(con.execute("SELECT ps_id FROM process_sheet WHERE selected_bom_id = ? ORDER BY ps_id", (bom_id,)))
...
def rebalance_after_row_actual(con, row_id, finishup_mode="next_free"):
    target = one(
        con.execute(
            """
            SELECT pr.ps_id, pr.seq_no, pr.machine_id, pr.flow_op_seq_id, pr.envelope_id, pr.locked, pr.split_piece_id,
                   pr.row_id, pr.plan_date, pr.start_min, pr.end_min, pr.qty, pr.actual_out,
                   pr.op_no, fs.cycle_time, fs.setup_time,
                   ps.total_qty, ps.selected_bom_id
            FROM planning_row pr
            JOIN operation_seq fs ON fs.op_seq_id = pr.flow_op_seq_id
            JOIN process_sheet ps ON ps.ps_id = pr.ps_id
            WHERE pr.row_id = ?
            """,
            (row_id,),
    ...
    if remaining_qty > 0:
        ...
        for row in refill_rows:
                con.execute(
                    """
                    INSERT INTO planning_row (envelope_id, split_piece_id, ps_id, plan_date, start_min, end_min, qty, setup_mins, seq_no, op_no, machine_id, machine_code, flow_op_seq_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (group_envelope_id, tail_piece_id, target["ps_id"], row["plan_date"], row["start_min"], row["end_min"], row["qty"], row["setup_mins"], target["seq_no"], target["op_no"], target["machine_id"], target["machine_code"], target["flow_op_seq_id"]),
                )
...
def rebalance_after_row_overrun_tail(con, row_id, delta_qty, finishup_mode="next_free"):
    target = one(
        con.execute(
            """
            SELECT pr.ps_id, pr.seq_no, pr.machine_id, pr.machine_code, pr.flow_op_seq_id, pr.envelope_id, pr.locked,
                   pr.row_id, pr.plan_date, pr.start_min, pr.end_min, pr.qty, pr.actual_out,
                   pr.op_no, fs.cycle_time, fs.setup_time,
                   ps.total_qty, ps.selected_bom_id
            FROM planning_row pr
    ...
    if qty_delta > 1e-9:
        ...
        for row in refill_rows:
            con.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (tail_envelope_id, tail_piece_id, target["ps_id"], row["plan_date"], row["start_min"], row["end_min"], row["qty"], row["setup_mins"], target["seq_no"], target["op_no"], target["machine_id"], target["machine_code"], target["flow_op_seq_id"]),
            )
        tail_row_ids = [
            int(r["row_id"])
            for r in rows(
...
def auto_plan(con, ps_id):
    ...
    has_step_overrides = any(step_completion_override(con, ps_id, step["op_seq_id"]) is not None for step in steps)
    all_steps_force_completed = bool(steps) and all(step_force_completed(con, ps_id, step["op_seq_id"]) for step in steps)
    ...
    planned = []
    skipped = []
    step_metrics = {row["op_seq_id"]: row for row in process_sheet_step_metrics(con, ps_id, steps)["steps"]}

    def step_remaining_qty(step):
        metric = step_metrics.get(step["op_seq_id"], {})
    ...
    while idx < len(steps):
        ...
        if len(chain) > 1:
            ...
            chain_remaining = {
                chain_step["op_seq_id"]: step_remaining_qty(chain_step)
                for chain_step in chain
        ...
        existing = existing_step_span(con, ps_id, step["op_seq_id"])
        metric = step_metrics.get(step["op_seq_id"], {})
        ...
        step_completed = step_force_completed(con, ps_id, step["op_seq_id"]) or step_actual >= ps["total_qty"]
        if step_completed:
...
@app.get("/planner")
def planner():
    return render_template("embedded_trial/trial.html")
...
@app.get("/materials")
def embedded_trial_materials():
    return render_template("embedded_trial/materials.html")


@app.get("/data-management")
@app.get("/data-management/")
def embedded_trial_data_management():
    with embedded_trial_db() as con:
        stats = embedded_trial_data_management_stats(con)
    return render_template("embedded_trial/data_management.html", stats=stats)


@app.route("/<page>")
def page(page):
    # Legacy planner.db pages kept for backup/transition. The canonical
    # scheduling entry is /planner and the new planner APIs are /api/trial/*.
    mapping = {
        ...
        "legacy-materials": "materials.html",
...
# Legacy planner.db APIs below are retained for old backup/transition pages
# only. New scheduling behavior must go through TRIAL/trial_app and /api/trial/*.
@app.get("/api/parts")
def api_parts():
    with db() as con:
        parts = rows(con.execute("SELECT * FROM parts ORDER BY part_name"))
        for part in parts:
            part["flows"] = rows(con.execute("SELECT bom_id, flow_code, is_default FROM bom_variation WHERE part_id = ? ORDER BY is_default DESC, bom_id", (part["part_id"],)))
...
def api_part_flows(part_id):
    with db() as con:
        ensure_part_flow_step_columns(con)
        flows = rows(con.execute("SELECT * FROM bom_variation WHERE part_id = ? ORDER BY is_default DESC, bom_id", (part_id,)))
        for flow in flows:
            flow["steps"] = flow_steps(con, flow["bom_id"])
...
def create_flow():
    ...
    with db() as con:
        ...
        return jsonify({"bom_id": cur.lastrowid})


@app.put("/api/flows/<int:bom_id>")
def update_flow(bom_id):
    data = request.get_json() or {}
    with db() as con:
        ensure_part_flow_step_columns(con)
        flow = one(con.execute("SELECT part_id FROM bom_variation WHERE bom_id = ?", (bom_id,)))
        if not flow:
        ...
        if data.get("is_default"):
            con.execute("UPDATE bom_variation SET is_default = 0 WHERE part_id = ?", (flow["part_id"],))
        con.execute(
            "UPDATE bom_variation SET flow_code = ?, flow_name = ?, is_default = ? WHERE bom_id = ?",
            (data.get("flow_code"), data.get("flow_name", ""), int(bool(data.get("is_default"))), bom_id),
        )
        existing_steps = rows(
            con.execute(
                """
                SELECT op_seq_id, seq_no, op_no, op_type, machine_category, preferred_machine, cycle_time, setup_time, is_last_op
                FROM operation_seq
                WHERE bom_id = ?
                ORDER BY seq_no, op_seq_id
                """,
        ...
        existing_by_id = {row["op_seq_id"]: row for row in existing_steps}
        existing_ids = set(existing_by_id.keys())
        incoming_ids = {int(step["op_seq_id"]) for step in steps if step.get("op_seq_id")}
        changed_from_seq = None

        removed_ids = [op_seq_id for op_seq_id in existing_ids if op_seq_id not in incoming_ids]
        if removed_ids:
            for op_seq_id in removed_ids:
                old_seq = int(existing_by_id[op_seq_id]["seq_no"])
                changed_from_seq = old_seq if changed_from_seq is None else min(changed_from_seq, old_seq)
            placeholders = ",".join("?" for _ in removed_ids)
            con.execute(
                f"UPDATE planning_block SET flow_op_seq_id = NULL WHERE flow_op_seq_id IN ({placeholders})",
                removed_ids,
            )
            con.execute(
                f"DELETE FROM operation_seq WHERE op_seq_id IN ({placeholders})",
        ...
        for idx, step in enumerate(steps, 1):
            op_seq_id = int(step["op_seq_id"]) if step.get("op_seq_id") else None
            params = (
                idx,
                step.get("op_no"),
                step.get("op_type"),
            ...
            if op_seq_id and op_seq_id in existing_ids:
                previous = existing_by_id[op_seq_id]
                if (
                ...
                changed_from_seq = idx if changed_from_seq is None else min(changed_from_seq, idx)
        refresh = apply_flow_timing_updates(con, bom_id, changed_from_seq)
        mark_flow_erp_sync_locked(con, bom_id, 1)
        return jsonify({"ok": True, **refresh})


@app.post("/api/flows/<int:bom_id>/duplicate")
def duplicate_flow(bom_id):
    with db() as con:
        ensure_part_flow_step_columns(con)
        flow = one(con.execute("SELECT * FROM bom_variation WHERE bom_id = ?", (bom_id,)))
        if not flow:
            return api_error("Flow not found", 404)
        code = f"{flow['flow_code']}-COPY"
        cur = con.execute(
            "INSERT INTO bom_variation (part_id, flow_code, flow_name, is_default) VALUES (?, ?, ?, 0)",
            (flow["part_id"], code, flow["flow_name"]),
        )
        for step in flow_steps(con, bom_id):
            con.execute(
                """
                INSERT INTO operation_seq (bom_id, seq_no, op_no, op_type, machine_category, preferred_machine, cycle_time, setup_time, is_last_op)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
def api_process_sheets():
    with db() as con:
        ...
        if q:
            result = [p for p in result if q in " ".join(str(p.get(k, "")).lower() for k in ("ps_id", "source_ps_id", "pp_partial_no", "part_name", "part_no", "part_desc"))]
...
def create_process_sheet():
    ...
    with db() as con:
        con.execute(
            """
            INSERT INTO process_sheet (ps_id, source_ps_id, pp_partial_no, part_id, selected_bom_id, part_no, part_desc, order_date, due_date, total_qty, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (ps_id, source_ps_id, partial_no, data.get("part_id"), data.get("selected_bom_id"), data.get("part_no", ""), data.get("part_desc", ""), data.get("order_date", ""), data.get("due_date", ""), data.get("total_qty"), data.get("status", "ACTIVE")),
        )
        con.execute(
...
def update_process_sheet(ps_id):
    ...
    with db() as con:
        ...
        con.execute(
            """
            UPDATE process_sheet SET source_ps_id = ?, pp_partial_no = ?, part_no = ?, part_desc = ?, order_date = ?, due_date = ?,
                   total_qty = ?, status = ?, selected_bom_id = ? WHERE ps_id = ?
            """,
            (source_ps_id, partial_no, data.get("part_no", ""), data.get("part_desc", ""), data.get("order_date", ""), data.get("due_date", ""), data.get("total_qty"), data.get("status", "ACTIVE"), data.get("selected_bom_id"), ps_id),
        )
        mark_ps_erp_sync_locked(con, ps_id, 1)
        return jsonify({"ok": True, "ps_id": ps_id, "source_ps_id": source_ps_id, "pp_partial_no": partial_no})
...
def set_process_sheet_complete(ps_id):
    ...
    with db() as con:
        ...
        if not ps:
            return api_error("Process sheet not found", 404)
        op_seq_ids = [row["op_seq_id"] for row in con.execute(
            """
            SELECT pfs.op_seq_id
            FROM process_sheet ps
            JOIN operation_seq pfs ON pfs.bom_id = ps.selected_bom_id
        ...
        if force_completed:
            ...
            for op_seq_id in op_seq_ids:
                con.execute(
                    ...
                    INSERT INTO process_sheet_step_local_override (ps_id, op_seq_id, force_completed, updated_at)
                    VALUES (?, ?, ?, CURRENT_TIMESTAMP)
                    ON CONFLICT(ps_id, op_seq_id) DO UPDATE SET force_completed = excluded.force_completed, updated_at = CURRENT_TIMESTAMP
                    """,
                    (ps_id, op_seq_id, 1),
                )
...
@app.post("/api/process-sheets/<ps_id>/steps/<int:op_seq_id>/complete")
def set_process_sheet_step_complete(ps_id, op_seq_id):
    data = request.get_json() or {}
    force_completed = 1 if data.get("force_completed") else 0
    with db() as con:
        ps = one(con.execute("SELECT ps_id FROM process_sheet WHERE ps_id = ?", (ps_id,)))
        step = one(con.execute("SELECT op_seq_id FROM operation_seq WHERE op_seq_id = ?", (op_seq_id,)))
        if not ps:
            return api_error("Process sheet not found", 404)
        if not step:
        ...
        con.execute(
            ...
            ON CONFLICT(ps_id, op_seq_id) DO UPDATE SET force_completed = excluded.force_completed, updated_at = CURRENT_TIMESTAMP
            """,
            (ps_id, op_seq_id, force_completed),
        )
        mark_ps_erp_sync_locked(con, ps_id, 1)
...
def auto_plan_op(con, ps_id, op_seq_id):
    ps = one(con.execute("SELECT ps.*, p.part_name FROM process_sheet ps JOIN parts p ON p.part_id = ps.part_id WHERE ps.ps_id = ?", (ps_id,)))
    if not ps:
        raise ValueError("Process sheet not found")

    steps = flow_steps(con, ps["selected_bom_id"])
    target = next((step for step in steps if int(step.get("op_seq_id") or 0) == int(op_seq_id or 0)), None)
    if not target:
        raise ValueError("Operation not found in selected flow")

    step_metrics = {row["op_seq_id"]: row for row in process_sheet_step_metrics(con, ps_id, steps)["steps"]}
    metric = step_metrics.get(target["op_seq_id"], {})
    planned_qty = float(metric.get("planned_qty") or 0)
    actual_qty = float(metric.get("actual_qty") or 0)
    ...
    if step_remaining_qty <= 0:
        return {
            "op_no": target["op_no"],
            "op_seq_id": target["op_seq_id"],
            "planned_qty": 0,
            "scheduled_rows": 0,
            "skipped": True,
        }

    existing = existing_step_span(con, ps_id, target["op_seq_id"])
    machine = None
...
@app.post("/api/plan/auto-op/<ps_id>/<int:op_seq_id>")
def api_auto_plan_op(ps_id, op_seq_id):
    with db() as con:
        try:
            result = auto_plan_op(con, ps_id, op_seq_id)
...
def api_unplan_unlocked(ps_id):
    with db() as con:
        ...
        group_keys = rows(
            con.execute(
                f"""
                SELECT DISTINCT ps_id, seq_no, op_no, machine_id, machine_code, flow_op_seq_id
                FROM planning_row
        ...
        if group_keys:
            for key in group_keys:
                con.execute(
                    ...
                    WHERE ps_id = ? AND seq_no = ? AND op_no = ? AND machine_id = ? AND machine_code = ? AND flow_op_seq_id = ?
                    """,
                    (key["ps_id"], key["seq_no"], key["op_no"], key["machine_id"], key["machine_code"], key["flow_op_seq_id"]),
                )
    ...
    with db() as con:
        deletable_blocks = rows(
            con.execute(
                ...
                FROM planning_envelope
                WHERE flow_op_seq_id IS NULL AND archived = 0
                """
            )
        )
    ...
    for legacy in legacy_rows:
        ...
        for row in step_rows:
            con.execute(
                ...
                INSERT INTO process_sheet_step_local_override (ps_id, op_seq_id, force_completed, updated_at)
                VALUES (?, ?, ?, CURRENT_TIMESTAMP)
                ON CONFLICT(ps_id, op_seq_id) DO UPDATE SET
                  force_completed = MAX(process_sheet_step_local_override.force_completed, excluded.force_completed),
                  updated_at = CURRENT_TIMESTAMP
                """,
                (canonical_ps_id, row["op_seq_id"], int(row["force_completed"] or 0)),
            )
    ...
    with db() as con:
        ...
        flows = rows(
            con.execute(
                ...
                FROM bom_variation pfh
                JOIN parts p ON p.part_id = pfh.part_id
                JOIN operation_seq pfs ON pfs.bom_id = pfh.bom_id
                WHERE pfh.flow_code LIKE 'BOM-%'
                ORDER BY pfh.bom_id DESC, pfs.seq_no
                LIMIT 40
                """
            )
        )
...
def legacy_trial_api_error():
    return api_error("Legacy trial API is disabled. Use the embedded TRIAL planner routes under /api/trial/*.", 410)
...
def api_trial_schedule():
    ...
    with trial_db() as con:
        include_completed = str(request.args.get("include_completed") or "").lower() in {"1", "true", "yes", "on"}
        trial_recalculate_all(con)
        machines = rows(con.execute("SELECT * FROM machines WHERE active = 1 ORDER BY machine_code"))
        machine_ids = [int(machine["machine_id"]) for machine in machines]
        blocks = rows(
            con.execute(
                """
                SELECT b.*, o.job_no, o.operation_name, o.total_qty, o.setup_minutes, o.cycle_minutes_per_qty,
                ...
                JOIN operation o ON o.operation_id = b.operation_id
                JOIN machines m ON m.machine_id = b.machine_id
                ORDER BY m.machine_code, b.queue_position, b.block_id
                """
            )
        )
        segments = rows(
            con.execute(
                ...
                ORDER BY s.machine_id, s.start_datetime, s.segment_id
                """
            )
        )
        actuals = rows(
        ...
        actual_totals = trial_actual_totals_by_block(con)
        for block in blocks:
            totals = actual_totals.get(int(block["block_id"]), {})
            if totals:
                block["actual_good_qty"] = totals.get("actual_good_qty", block["actual_good_qty"])
                block["actual_reject_qty"] = totals.get("actual_reject_qty", block["actual_reject_qty"])
        capacities = rows(
            con.execute(
                f"""
                SELECT tmd.*, tcp.profile_name, m.machine_code
                FROM machine_capacity_day tmd
                JOIN capacity_profile tcp ON tcp.profile_id = tmd.profile_id
                JOIN machines m ON m.machine_id = tmd.machine_id
                WHERE tmd.machine_id IN ({','.join('?' for _ in machine_ids) if machine_ids else '0'})
                ORDER BY m.machine_code, tmd.work_date
                """,
                machine_ids if machine_ids else [],
            )
        ) if machine_ids else []
        profiles = rows(con.execute("SELECT * FROM capacity_profile ORDER BY profile_name"))
        catalog_data = trial_catalog_items(con, include_completed=include_completed)
        return jsonify({
            "machines": machines,
            "blocks": blocks,
            "segments": segments,
            ...
            "profiles": profiles,
            "catalog": catalog_data.get("available", []),
            "planned": catalog_data.get("planned", []),
        })


@app.post("/api/legacy-trial/operations")
def api_trial_create_operation():
    return legacy_trial_api_error()
    data = request.get_json() or {}
    job_no = compact_text(data.get("job_no"))
    operation_name = compact_text(data.get("operation_name"))
    source_ps_id = compact_text(data.get("source_ps_id"))
    ...
    if not job_no or not operation_name:
        return api_error("Job number and operation name are required")
    machine_id = int(data.get("machine_id") or 0)
    if not machine_id:
        return api_error("Machine is required")
    scheduled_qty = parse_number(data.get("scheduled_qty"), parse_number(data.get("total_qty"), 0))
    with trial_db() as con:
        operation_cur = con.execute(
            """
            ...
            (
                job_no,
                operation_name,
                parse_number(data.get("total_qty"), scheduled_qty),
        ...
        operation_id = operation_cur.lastrowid
        queue_position = int(data.get("queue_position") or 0)
        if queue_position <= 0:
            queue_position = 1 + int(con.execute("SELECT COALESCE(MAX(queue_position), 0) FROM run_block WHERE machine_id = ?", (machine_id,)).fetchone()[0] or 0)
        block_cur = con.execute(
            """
            INSERT INTO run_block (
              operation_id, machine_id, queue_position, scheduled_qty, include_setup, status, anchor_datetime,
            ...
            (
                ...
                queue_position,
                scheduled_qty,
                int(data.get("include_setup", 1)),
                compact_text(data.get("status") or "PLANNED") or "PLANNED",
                compact_text(data.get("anchor_datetime")),
                compact_text(data.get("remarks")),
            ),
        )
        trial_recalculate_machine(con, machine_id)
        block = trial_block_row(con, block_cur.lastrowid)
        return jsonify({"ok": True, "operation_id": operation_id, "block": trial_block_payload(block)})


@app.put("/api/legacy-trial/blocks/<int:block_id>")
def api_trial_update_block(block_id):
    ...
    with trial_db() as con:
        ...
        if not block:
            return api_error("Run block not found", 404)
        op_updates = {}
        ...
        if "status" in data:
            block_updates["status"] = compact_text(data.get("status")) or "PLANNED"
        ...
        if "block_remarks" in data:
            block_updates["remarks"] = compact_text(data.get("block_remarks"))
        if block_updates:
            set_clause = ", ".join(f"{key} = ?" for key in block_updates)
            con.execute(
                f"UPDATE run_block SET {set_clause}, updated_at = CURRENT_TIMESTAMP WHERE block_id = ?",
                (*block_updates.values(), block_id),
            )
        ...
        for machine_id in machine_ids:
            trial_recalculate_machine(con, machine_id)
        block = trial_block_row(con, block_id)
        return jsonify({"ok": True, "block": trial_block_payload(block)})


@app.post("/api/legacy-trial/blocks/<int:block_id>/split")
def api_trial_split_block(block_id):
    return legacy_trial_api_error()
    data = request.get_json() or {}
    split_qty = parse_number(data.get("split_qty"), 0)
    if split_qty <= 0:
        return api_error("Split quantity is required")
    with trial_db() as con:
        block = trial_block_row(con, block_id)
        if not block:
            return api_error("Run block not found", 404)
        if split_qty >= float(block["scheduled_qty"] or 0):
            return api_error("Split quantity must be smaller than the scheduled quantity")
        remaining = float(block["scheduled_qty"] or 0) - split_qty
        max_position = int(con.execute("SELECT COALESCE(MAX(queue_position), 0) FROM run_block WHERE machine_id = ?", (int(block["machine_id"]),)).fetchone()[0] or 0)
        con.execute(
        ...
        cur = con.execute(
            ...
            VALUES (?, ?, ?, ?, ?, 'PLANNED', '', '', '', 0, 0, ?, CURRENT_TIMESTAMP)
            """,
            (
        ...
        trial_recalculate_machine(con, int(block["machine_id"]))
...
@app.delete("/api/legacy-trial/blocks/<int:block_id>")
def api_trial_delete_block(block_id):
    ...
    with trial_db() as con:
        ...
        remaining_blocks = one(
            con.execute(
                "SELECT COUNT(*) AS cnt FROM run_block WHERE operation_id = ?",
                (operation_id,),
            )
        )
        if int((remaining_blocks or {}).get("cnt") or 0) <= 0:
            con.execute("DELETE FROM operation WHERE operation_id = ?", (operation_id,))
        trial_recalculate_machine(con, machine_id)
        return jsonify({"ok": True})


@app.post("/api/legacy-trial/capacity")
def api_trial_capacity():
    return legacy_trial_api_error()
    data = request.get_json() or {}
    work_date = compact_text(data.get("work_date"))
    if not work_date:
        return api_error("Work date is required")
    ...
    with trial_db() as con:
        ...
        if not machine_ids:
            return api_error("No active machines available", 404)
        profile = None
        if profile_name:
            profile = capacity_profile_row(con, profile_name)
        if not profile:
            capacity_minutes = int(parse_number(data.get("capacity_minutes"), 0))
            start_minute = int(parse_number(data.get("start_minute"), 510))
            if not profile_name:
                profile_name = f"CUSTOM_{capacity_minutes}_{start_minute}"
            profile = capacity_profile_row(con, profile_name)
            if not profile:
                cur = con.execute(
                    "INSERT INTO capacity_profile (profile_name, capacity_minutes, start_minute, note) VALUES (?, ?, ?, ?)",
                    (profile_name, capacity_minutes, start_minute, compact_text(data.get("note"))),
                )
                profile = capacity_profile_row(con, profile_name)
        for machine_id in machine_ids:
            existing = one(con.execute("SELECT day_id FROM machine_capacity_day WHERE machine_id = ? AND work_date = ?", (machine_id, work_date)))
            payload = (
                machine_id,
                work_date,
            ...
            if existing:
                con.execute(
                    """
                    UPDATE machine_capacity_day
                    SET profile_id = ?, capacity_minutes = ?, start_minute = ?, note = ?, updated_at = CURRENT_TIMESTAMP
                    WHERE machine_id = ? AND work_date = ?
                    """,
                    (int(profile["profile_id"]), int(profile["capacity_minutes"] or 0), int(profile["start_minute"] or 0), compact_text(data.get("note")), machine_id, work_date),
                )
            else:
                con.execute(
                    """
                    INSERT INTO machine_capacity_day (machine_id, work_date, profile_id, capacity_minutes, start_minute, note)
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    payload,
                )
...
@app.post("/api/legacy-trial/blocks/<int:block_id>/actual")
def api_trial_actual(block_id):
    ...
    with trial_db() as con:
        ...
        action = compact_text(data.get("action"))
        scheduled_qty = float(block["scheduled_qty"] or 0)
        end_dt = trial_parse_dt(block.get("calculated_end_datetime"))
        span_one_day = True
        if block.get("calculated_start_datetime") and block.get("calculated_end_datetime"):
            start_dt = trial_parse_dt(block.get("calculated_start_datetime"))
            span_one_day = bool(start_dt and end_dt and start_dt.date() == end_dt.date())
        daily_actuals = data.get("daily_actuals") or []
        delete_actual_dates = data.get("delete_actual_dates") or []
        normalized_daily_actuals = []
        if isinstance(daily_actuals, list) and daily_actuals:
            for row in daily_actuals:
                ...
                if not report_date:
                    continue
                normalized_daily_actuals.append(
                    {
                        "report_date": report_date,
                        "actual_good_qty": max(0.0, parse_number(row.get("actual_good_qty"), 0)),
                        "actual_reject_qty": max(0.0, parse_number(row.get("actual_reject_qty"), 0)),
                        "remarks": compact_text(row.get("remarks")) or remarks,
                    }
                )
        delete_actual_dates = [
            compact_text(value)
            for value in (delete_actual_dates if isinstance(delete_actual_dates, list) else [])
            if compact_text(value)
        ]
        if normalized_daily_actuals:
            actual_good_qty = sum(float(row["actual_good_qty"] or 0) for row in normalized_daily_actuals)
            actual_reject_qty = sum(float(row["actual_reject_qty"] or 0) for row in normalized_daily_actuals)
        else:
            raw_good_qty = data.get("actual_good_qty")
            raw_reject_qty = data.get("actual_reject_qty")
            actual_good_qty = max(0.0, parse_number(raw_good_qty, 0))
            actual_reject_qty = max(0.0, parse_number(raw_reject_qty, 0))
        excess = actual_good_qty - scheduled_qty
        if excess > 0 and action != "accept_extra":
            return api_error(
                f"Actual exceeds remaining quantity by {excess:g}. Correct input or accept extra output.",
                409,
                {"needs_decision": True, "excess": excess},
            )
        if actual_good_qty < scheduled_qty and span_one_day and action not in {"continue", "split"}:
            return api_error(
                "This block was planned to finish today. Choose continue or split the remaining quantity.",
                409,
                {"needs_decision": True, "remaining": scheduled_qty - actual_good_qty},
            )
        if action == "split" and actual_good_qty < scheduled_qty:
            remaining = scheduled_qty - actual_good_qty
            max_position = int(con.execute("SELECT COALESCE(MAX(queue_position), 0) FROM run_block WHERE machine_id = ?", (int(block["machine_id"]),)).fetchone()[0] or 0)
            con.execute(
                "UPDATE run_block SET scheduled_qty = ?, actual_good_qty = ?, actual_reject_qty = ?, remarks = ?, status = 'DONE', updated_at = CURRENT_TIMESTAMP WHERE block_id = ?",
                (actual_good_qty, actual_good_qty, actual_reject_qty, remarks, block_id),
            )
            con.execute(
                """
                INSERT INTO run_block (
            ...
            new_status = "DONE" if actual_good_qty >= scheduled_qty else "IN_PROGRESS"
            con.execute(
                """
                UPDATE run_block
                SET actual_good_qty = ?, actual_reject_qty = ?, remarks = ?, status = ?, updated_at = CURRENT_TIMESTAMP
                WHERE block_id = ?
                """,
                (actual_good_qty, actual_reject_qty, remarks, new_status, block_id),
            )
        if normalized_daily_actuals:
            reported_by = compact_text(data.get("reported_by"))
            for row in normalized_daily_actuals:
                con.execute(
                    "DELETE FROM production_actual WHERE block_id = ? AND report_date = ?",
                    (block_id, row["report_date"]),
                )
                con.execute(
                    """
                    INSERT INTO production_actual (block_id, report_date, actual_good_qty, actual_reject_qty, remarks, reported_by)
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (
                        block_id,
            ...
            con.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (block_id, compact_text(data.get("report_date")) or date.today().isoformat(), actual_good_qty, actual_reject_qty, remarks, compact_text(data.get("reported_by"))),
            )
        if delete_actual_dates:
            q_marks = ",".join("?" for _ in delete_actual_dates)
            con.execute(
                f"DELETE FROM production_actual WHERE block_id = ? AND report_date IN ({q_marks})",
                (block_id, *delete_actual_dates),
            )
        trial_recalculate_machine(con, int(block["machine_id"]))
...
def api_materials():
    with db() as con:
        ...
        for ps in planned_support_process_sheets(con):
            ...
            mat["part_desc"] = ps.get("part_desc", "")
...
# Legacy planner.db Summary route; do not use for new TRIAL scheduler.
@app.get("/api/summary")
def api_summary():
...
def machine_summary(con, start, end, machine_category=None):
    ...
    active_rows = rows(con.execute(
        ...
        JOIN operation_seq fs ON fs.op_seq_id = pe.flow_op_seq_id
        JOIN machines m ON m.machine_id = pe.machine_id
        WHERE pe.archived = 0
    ...
    for row in all_rows:
        ...
        flow_op_seq_id = int(row.get("flow_op_seq_id") or 0)
        detail_key = (ps_key, seq_no, op_no, flow_op_seq_id)
...
# Legacy planner.db Gantt route. Kept only for older backup/transition screens;
# the TRIAL scheduler should use /api/trial/gantt instead.
@app.get("/api/gantt")
def api_gantt():
    start = request.args.get("from", date.today().isoformat())
    ...
    with db() as con:
        ...
        blocks = rows(con.execute(
            """
            SELECT COALESCE(pe.envelope_id, pr.envelope_id, 0) AS envelope_id,
                   COALESCE(pe.envelope_id, pr.envelope_id, 0) AS display_env_id, NULL AS hist_envelope_id,
                   CASE WHEN COALESCE(pe.locked, 0) = 1 OR COALESCE(pr.locked, 0) = 1 THEN 1 ELSE 0 END AS locked,
                   COALESCE(pe.machine_id, pr.machine_id, 0) AS machine_id,
                   pr.row_id, pr.plan_date, pr.start_min, pr.end_min,
                   pr.qty row_qty, pr.actual_out, pr.actual_out_set, pr.setup_mins, pr.locked AS row_locked,
                   COALESCE((SELECT SUM(pr2.qty) FROM planning_row pr2 WHERE pr2.envelope_id = COALESCE(pe.envelope_id, pr.envelope_id)), pr.qty, 0) AS envelope_qty_all,
                   ps.ps_id, ps.source_ps_id, ps.pp_partial_no, ps.due_date,
                   COALESCE(fs.op_no, pr.op_no) AS op_no, COALESCE(fs.op_type, pr.op_no) AS op_type,
                   COALESCE(pe.seq_no, pr.seq_no, 0) AS seq_no, COALESCE(m.machine_code, pr.machine_code) AS machine_code,
                   0 AS is_history, 0 AS archived_view
            FROM planning_row pr
            LEFT JOIN planning_envelope pe ON pe.envelope_id = pr.envelope_id
            JOIN process_sheet ps ON ps.ps_id = pr.ps_id
            LEFT JOIN operation_seq fs ON fs.op_seq_id = COALESCE(pe.flow_op_seq_id, pr.flow_op_seq_id)
            LEFT JOIN machines m ON m.machine_id = COALESCE(pe.machine_id, pr.machine_id)
            WHERE pr.plan_date BETWEEN ? AND ? AND COALESCE(pe.archived, 0) = 0
...
def _sync_envelope_summary_from_envelope_id(con, envelope_id):
    ...
    envelope_rows = rows(
        con.execute(
            """
            SELECT row_id, ps_id, plan_date, start_min, end_min, qty, locked, seq_no, op_no, machine_id, machine_code, flow_op_seq_id, split_group_id, split_piece_id
            FROM planning_row
            WHERE envelope_id = ?
...
def _rows_for_group_key(con, group_key):
    ps_id, seq_no, op_no, machine_id, machine_code, flow_op_seq_id = group_key
    return rows(
        con.execute(
            """
            SELECT *
...
def _sync_group_locks_from_row_ids(con, row_ids):
    ...
    rows_in_scope = rows(
        con.execute(
            f"""
            SELECT row_id, ps_id, plan_date, start_min, end_min, qty, locked, seq_no, op_no, machine_id, machine_code, flow_op_seq_id, envelope_id, split_group_id, split_piece_id
            FROM planning_row
            WHERE row_id IN ({q_marks})
            ORDER BY plan_date, start_min, row_id
            """,
        ...
        for row in group_rows:
            ...
            envelope = one(
                con.execute(
                    ...
                    LIMIT 1
                    """,
                    (ps_id, seq_no, op_no, machine_id, machine_code, flow_op_seq_id, run_start, run_end),
                )
            )
            if envelope:
                ...
                con.execute(
                    """
                    INSERT INTO planning_envelope (ps_id, seq_no, op_no, machine_id, machine_code, flow_op_seq_id, split_group_id, split_piece_id, total_qty, locked, archived, envelope_start, envelope_end)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, 0, ?, ?)
                    """,
                    (ps_id, seq_no, op_no, machine_id, machine_code, flow_op_seq_id, split_group_id, group_rows[0].get("split_piece_id") or "", locked, run_start, run_end),
                )
                envelope = one(
                    con.execute(
                        """
        ...
        def saved_row_snapshot(snapshot_row_id):
            snapshot = one(
                con.execute(
                    """
                    SELECT row_id, envelope_id, ps_id, plan_date, start_min, end_min, qty, actual_out, actual_out_set,
                           seq_no, op_no, machine_id, machine_code, flow_op_seq_id, split_group_id, split_piece_id
                    FROM planning_row
                    WHERE row_id = ?
                    """,
                    (int(snapshot_row_id),),
        ...
        row = one(
            con.execute(
                ...
                SELECT row_id, ps_id, plan_date, start_min, end_min, qty, actual_out, actual_out_set,
                       setup_mins, locked, seq_no, op_no, machine_id, machine_code, flow_op_seq_id, envelope_id
                FROM planning_row
                WHERE row_id = ?
                """,
                (int(row_id),),
        ...
        with db() as con:
            ...
            selected_row = one(
                con.execute(
                    """
                    SELECT row_id, ps_id, plan_date, start_min, end_min, qty, actual_out, actual_out_set,
                           locked, seq_no, op_no, machine_id, machine_code, flow_op_seq_id, envelope_id, split_piece_id
                    FROM planning_row
                    WHERE row_id = ?
                    """,
                    (int(row_id),),
            ...
            step = one(con.execute("SELECT * FROM operation_seq WHERE op_seq_id = ?", (int(primary_row["flow_op_seq_id"] or 0),)))
            if not step:
...
def move_row_qty_by_id(data=None):
    try:
        ...
        with db() as con:
            ...
            step = one(con.execute("SELECT * FROM operation_seq WHERE op_seq_id = ?", (int(anchor_row["flow_op_seq_id"] or 0),)))
    ...
    with db() as con:
        ...
        if remaining_row_ids:
            ...
            remaining_rows = rows(
                con.execute(
                    f"""
                    SELECT row_id, plan_date, start_min, end_min, qty, setup_mins, seq_no, op_no, machine_id, machine_code, flow_op_seq_id, split_group_id, split_piece_id
                    FROM planning_row
                    WHERE row_id IN ({q_marks})
                    ORDER BY plan_date, start_min, end_min, row_id
                    """,
...
def merge_row_into_row(source_row_id, target_row_id):
    with db() as con:
        ...
        if not source_envelope or not target_envelope:
            return api_error("Envelope row not found", 404)
        if compact_text(source_row["ps_id"]) != compact_text(target_row["ps_id"]) or int(source_row["flow_op_seq_id"]) != int(target_row["flow_op_seq_id"]) or int(source_row["machine_id"]) != int(target_row["machine_id"]):
        ...
        step = one(
            con.execute(
                """
                SELECT op_seq_id, seq_no, op_no, cycle_time, setup_time
                FROM operation_seq
                WHERE op_seq_id = ?
                """,
                (int(earliest_row["flow_op_seq_id"]),),
            )
        )
        machine = one(
            con.execute(
...
def archive_row_by_id(data=None):
    ...
    with db() as con:
        ...
        row = one(con.execute(
            """
            SELECT row_id, ps_id, plan_date, start_min, end_min, qty, actual_out, actual_out_set,
                   seq_no, op_no, machine_id, machine_code, flow_op_seq_id, envelope_id
            FROM planning_row
            WHERE row_id = ?
            """,
            (int(row_id),),
        ...
        con.execute(
            "INSERT INTO history_envelope (ps_id, seq_no, op_no, machine_id, machine_code, flow_op_seq_id, total_qty, actual_out, locked, archived_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)",
            (row["ps_id"], row["seq_no"], row["op_no"], row.get("machine_id") or 0, row["machine_code"], row.get("flow_op_seq_id") or 0, row["qty"], row["actual_out"], row["locked"] or 0),
        )
        hist_envelope_id = con.execute("SELECT last_insert_rowid()").fetchone()[0]
        con.execute(
            "INSERT INTO history_row (hist_envelope_id, ps_id, plan_date, start_min, end_min, qty, actual_out, actual_out_set, seq_no, op_no, machine_id, machine_code, flow_op_seq_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (hist_envelope_id, row["ps_id"], row["plan_date"], row["start_min"], row["end_min"], row["qty"], row["actual_out"], row["actual_out_set"], row["seq_no"], row["op_no"], row.get("machine_id") or 0, row["machine_code"], row.get("flow_op_seq_id") or 0),
        )
