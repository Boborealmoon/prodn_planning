...
CREATE TABLE IF NOT EXISTS part_flow_header (
...
CREATE TABLE IF NOT EXISTS part_flow_steps (
  step_id INTEGER PRIMARY KEY AUTOINCREMENT,
  flow_id INTEGER NOT NULL REFERENCES part_flow_header(flow_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS process_sheet (
  ...
  selected_flow_id INTEGER REFERENCES part_flow_header(flow_id),
...
CREATE TABLE IF NOT EXISTS planning_block (
  ...
  flow_step_id INTEGER REFERENCES part_flow_steps(step_id),
  op_no TEXT NOT NULL,
  seq_no INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS planning_envelope (
  ...
  created_at TEXT DEFAULT CURRENT_TIMESTAMP,
  updated_at TEXT DEFAULT CURRENT_TIMESTAMP,
  UNIQUE (ps_id, seq_no, op_no, machine_id, machine_code, flow_step_id, split_piece_id)
);
...
CREATE TABLE IF NOT EXISTS history_envelope (
  ...
  locked INTEGER DEFAULT 0,
  archived_at TEXT DEFAULT CURRENT_TIMESTAMP,
  UNIQUE (ps_id, seq_no, op_no, machine_id, machine_code, flow_step_id)
);
...
CREATE TABLE IF NOT EXISTS process_sheet_step_local_override (
  ps_id TEXT NOT NULL REFERENCES process_sheet(ps_id) ON DELETE CASCADE,
  step_id INTEGER NOT NULL REFERENCES part_flow_steps(step_id) ON DELETE CASCADE,
  force_completed INTEGER NOT NULL DEFAULT 0,
  updated_at TEXT DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (ps_id, step_id)
);
...
SELECT flow_id, part_id, flow_code, flow_name, is_default FROM part_flow_header;

CREATE VIEW IF NOT EXISTS flow_steps AS
SELECT step_id, flow_id, seq_no AS seq, op_no, op_type, machine_category,
       preferred_machine, cycle_time, setup_time, is_last_op
FROM part_flow_steps;

CREATE VIEW IF NOT EXISTS process_sheets AS
SELECT ps_id, part_id, selected_flow_id, inv_code, inv_desc, order_date, due_date,
...
SELECT block_id, ps_id, flow_step_id AS step_id, machine_id, locked, archived
FROM planning_block;
...
SELECT row_id, block_id, envelope_id, ps_id, plan_date, start_min, end_min, qty, actual_out, actual_out_set, setup_mins,
       locked, seq_no, op_no, machine_id, machine_code, flow_step_id
FROM planning_row;

CREATE VIEW IF NOT EXISTS planning_envelopes AS
SELECT envelope_id, ps_id, seq_no, op_no, machine_id, machine_code, flow_step_id,
...
SELECT hist_envelope_id, ps_id, seq_no, op_no, machine_id, machine_code, flow_step_id,
...
BEGIN
  INSERT INTO part_flow_header (part_id, flow_code, flow_name, is_default)
...
BEGIN
  ...
  VALUES (NEW.flow_id, NEW.seq, NEW.op_no, NEW.op_type, NEW.machine_category, NEW.preferred_machine, NEW.cycle_time, NEW.setup_time, COALESCE(NEW.is_last_op, 0));
END;

CREATE TRIGGER IF NOT EXISTS flow_steps_delete INSTEAD OF DELETE ON flow_steps
BEGIN
  DELETE FROM part_flow_steps WHERE step_id = OLD.step_id;
END;

CREATE TRIGGER IF NOT EXISTS process_sheets_insert INSTEAD OF INSERT ON process_sheets
BEGIN
  INSERT INTO process_sheet (ps_id, part_id, selected_flow_id, inv_code, inv_desc, order_date, due_date, total_qty, status)
  VALUES (NEW.ps_id, NEW.part_id, NEW.selected_flow_id, NEW.inv_code, NEW.inv_desc, NEW.order_date, NEW.due_date, NEW.total_qty, COALESCE(NEW.status, 'ACTIVE'));
END;
...
BEGIN
  ...
  SET inv_code = NEW.inv_code, inv_desc = NEW.inv_desc, order_date = NEW.order_date,
      ...
      selected_flow_id = NEW.selected_flow_id, updated_at = CURRENT_TIMESTAMP
...
BEGIN
  INSERT INTO planning_block (ps_id, flow_step_id, op_no, seq_no, machine_id, machine_code, total_qty, locked, archived)
  SELECT NEW.ps_id, NEW.step_id, fs.op_no, fs.seq_no, NEW.machine_id, m.machine_code, 0, COALESCE(NEW.locked, 0), COALESCE(NEW.archived, 0)
  FROM part_flow_steps fs, machines m
  WHERE fs.step_id = NEW.step_id AND m.machine_id = NEW.machine_id;
  INSERT OR IGNORE INTO planning_envelope (ps_id, seq_no, op_no, machine_id, machine_code, flow_step_id, total_qty, locked, archived, envelope_start, envelope_end)
  SELECT NEW.ps_id, fs.seq_no, fs.op_no, NEW.machine_id, m.machine_code, NEW.step_id, 0, COALESCE(NEW.locked, 0), COALESCE(NEW.archived, 0), '', ''
  FROM part_flow_steps fs, machines m
  WHERE fs.step_id = NEW.step_id AND m.machine_id = NEW.machine_id;
END;
...
BEGIN
  INSERT INTO planning_row (block_id, envelope_id, split_piece_id, ps_id, plan_date, start_min, end_min, qty, actual_out, actual_out_set, setup_mins, seq_no, op_no, machine_id, machine_code, flow_step_id)
  SELECT
      NEW.block_id,
      COALESCE((
          ...
          WHERE pe.ps_id = pb.ps_id
            ...
            AND pe.flow_step_id = pb.flow_step_id
          LIMIT 1
      ), 0),
      COALESCE((
          ...
          JOIN planning_block pb ON pb.ps_id = pe.ps_id
                              ...
                              AND pb.flow_step_id = pe.flow_step_id
...
BEGIN
  ...
  INSERT INTO history_envelope (ps_id, seq_no, op_no, machine_id, machine_code, flow_step_id, total_qty, actual_out, locked, archived_at)
  VALUES (NEW.ps_id, 0, NEW.op_no, 0, NEW.machine_code, 0, NEW.row_qty, NEW.row_actual, 0, CURRENT_TIMESTAMP);
  INSERT INTO history_row (hist_block_id, hist_envelope_id, ps_id, plan_date, start_min, end_min, qty, actual_out, actual_out_set, seq_no, op_no, machine_id, machine_code, flow_step_id)
  VALUES (
